...
**5. Write the source as TEXT, then save it.** The page goes in a ```vue fenced block in your message — the complete SFC, written once — and in that same message you call `managePage { action: "stage" }`. Next message, save it: `create` (or `update { pageId }` for a page that exists) with `definition.code.source` set to exactly `"@emitted"`, plus the definition sections that go with it. The fence is read back from your own message; you never write the code twice.

Source pasted into a tool argument NEVER arrives when the file is large. It is buffered whole before anything is sent, so the connection sits silent for the minutes you spend writing and is cut before the argument exists — measured ten times in one night, on two different models, always the same shape: the page written in full, paid for, and lost. Text streams as you write it, so the same file arrives every time.

The save itself compiles, runs the datasets and returns samples and warnings — that IS your dry run, so never `dry_run` code you are about to save or just saved. A failed save is not a rewrite either: when compile refuses the write the source is KEPT, and you fix the named lines with `update { edits }` against it.

A big page is written differently, and getting this wrong is how one comes out truncated. The ceiling is not the page's — a source may run to 240 000 characters — it is how much you can write in ONE answer. So past about four views, or when you can see the file running long: emit and save a **complete, compilable skeleton** — the real layout, the real datasets, every region present as a visible placeholder, and one unique marker per region (`<!-- SECTION:board -->` in the template, `// SECTION:board` in the script). Then fill one section per `update { edits }`, replacing its marker — each update compiles and reports as it lands. **Review only once the last marker is gone.** A skeleton that compiles is a page you can always hand over; half an SFC is nothing at all. Reviewing section by section spends the whole budget judging a page that does not exist yet.
