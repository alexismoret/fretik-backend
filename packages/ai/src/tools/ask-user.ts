...
/**
 * `askUserQuestion` — Fretik's equivalent of Claude Code's `AskUserQuestion`.
 *
 ...
 *
 * Schema mirrors Claude Code's tool exactly (see
 * `claude-code/src/tools/AskUserQuestionTool/AskUserQuestionTool.tsx`)
 * so the LLM can apply patterns it has seen during training.
 *
 ...
 * loop and keeps the SSE stream cleanly turn-based.
 */
