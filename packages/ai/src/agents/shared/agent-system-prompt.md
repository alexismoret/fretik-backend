...
- **searchWeb(queries, depth?, filters?)** — Public web search. Pass 2-3 phrasings of ONE question in a single call — they run together for one price. Date, domain, language and country filters; `academic` and `sec` verticals. Search whenever a fact is uncertain, whatever the subject — but the team's own data comes from the internal tools first.
