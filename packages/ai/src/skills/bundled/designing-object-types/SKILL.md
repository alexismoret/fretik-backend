...
- `rating` — a 1–N star/icon score. Config: `ratingMax` (default 5), `ratingIcon`.
...
- **`relation` vs `member`:** `member` is an internal teammate; `relation` links to another object type's records. A relation is added with `manageField` `type:'relation'`; changing a field to/from relation isn't supported — recreate it.
...
Never set `color` on a type or option — the system auto-assigns a distinct palette color. Set one only when the user explicitly names a color.
...
3. manageObjectType create → key + label + description + icon + the WHOLE fields[] in one call
   ...
   - select/multi_select fields: options with label + icon (+ group for status)
...
- `objects.records.bulk_create(type_key, rows)` — `rows` = list of field maps. Returns `{ids, okCount, errors}`; `ids[i]` aligns with `rows[i]`.
