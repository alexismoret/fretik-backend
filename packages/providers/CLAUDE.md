...
5. Add `guidance.md` — instructions the chatbot sees once it touches this provider. **Follows `.agent/agent-context-framework.md` and explicitly does NOT duplicate the persona/voice section (which the generator injects automatically for `categories.includes("communication")`).**
