...
/**
 * Domain tool (deferred) — find valid icon names for collections / select
...
export const createSearchIconsTool = () =>
  tool({
    description:
      "Find icon names from the curated Lucide catalog. Pass every concept you need icons for at once (e.g. ['project','invoice','urgent']); returns ranked names per concept. Use the chosen `name` with manageCollection / manageField, which reject names outside the catalog.",
    ...
    execute: async ({ queries, limit }) => ({
      results: queries.map((query) => ({
        ...
        // manageCollection / manageField); `searchIcons` returns {name,tags}.
