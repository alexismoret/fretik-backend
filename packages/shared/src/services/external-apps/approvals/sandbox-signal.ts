 ...
 * sandbox-exec dispatcher (API process) and the `python` tool (AI process).
 *
 ...
 * the tool only emits that when `fretik_apps.run_plan(...)` raises
 * `ApprovalPending` **uncaught**. If the agent wraps `run_plan` in
 * `try/except` (or otherwise swallows the exception), the row is still
 * created server-side but the UI never learns about it — the user sees no
 * modal and the write silently hangs.
 *
 * The dispatcher is the single source of truth for "a pending approval was
 * returned this run", so it stamps this Redis key whenever it returns
 * `approval_pending`. The `python` tool consumes (read-once) the key right
 * after the SAME cell finishes and surfaces the approval regardless of how
 * the agent handled the exception.
 *
 ...
 * the few seconds between "dispatcher created the row" and "python cell
 * returns", bounded by the 5-min sandbox cap. It is set during the
 * `run_plan` call and deleted on read in the same cell, so the TTL is just a
 * backstop for a cell that dies before consuming it — short on purpose, so a
