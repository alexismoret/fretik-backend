...
const runExtractionForFile = async (args: {
  ...
  try {
    // Mistral OCR needs a presigned URL reachable from its servers —
    // generate one with enough headroom to cover queueing + OCR
    // roundtrip on a large PDF. Spreadsheet / text branches ignore
    // this URL and work on `bytes` directly.
    const presignedUrl = await getPresignedUrl(args.s3Key, 60 * 10);
    ...
    const writeSidecar = shouldWriteSidecar(args.mimeType, result.content);
    if (writeSidecar) {
      ...
      await uploadContextSidecar(args.profileId, args.fileId, result.content);
    }
    ...
    await db
      ...
      .set({
        ...
        charCount: result.charCountFull,
