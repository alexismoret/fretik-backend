...
export const createManageWorkflowTool = () =>
  tool({
    description: [
      ...
      "",
      "Set `triggerCriterion` on every event trigger, written from the playbook's goal: firings that fail it become `filtered` runs instead of full ones. Omit it only when every firing deserves a run.",
      "",
