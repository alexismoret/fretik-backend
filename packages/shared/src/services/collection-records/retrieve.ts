...
import { computeRelationRollupValues } from "../collection-schema/computed";
...
export type RecordLinkSummary = {
  ...
  toRecord: { id: string; label: string; collectionId: string };
};

export type CollectionRecordListItem = CollectionRecordWithData & {
...
const fetchOutgoingLinkSummaries = async (
  ...
  const rows = await db
    .select({
      ...
      toId: toRec.id,
      toLabel: toRec.label,
      toCollectionId: toRec.collectionId,
    })
    .from(links)
...
const SORTABLE_COLUMNS = {
  ...
  updatedAt: collectionRecords.updatedAt,
} as const;

/**
 * System-property field types → the `collection_records` registry column they
...
const SYSTEM_FIELD_COLUMN: Partial<Record<FieldDefinitionType, SQL>> = {
  created_time: sql`${collectionRecords.createdAt}`,
  last_edited_time: sql`${collectionRecords.updatedAt}`,
  created_by: sql`${collectionRecords.createdByUserId}`,
  last_edited_by: sql`${collectionRecords.updatedByUserId}`,
};
...
const resolveSortExpression = (
  ...
  if (sortBy.startsWith("field:")) {
    ...
    if (SLUG.test(key)) {
      ...
      return sql`(SELECT e.${sql.raw(`"${colName}"`)} FROM ${sql.raw(table)} e WHERE e."id" = ${collectionRecords.id})`;
    }
    return collectionRecords.createdAt;
  }
  return (
...
const buildFilterCondition = (
  ...
  return sql`EXISTS (SELECT 1 FROM ${table} e WHERE e."id" = ${collectionRecords.id} AND ${pred})`;
};
...
export const listCollectionRecords = async (data: {
   ...
   *  advancing by result COUNT (the `listRecords` tool) lands between page
  ...
  if (search && search.trim().length > 0) {
    ...
    const arms = [
      sql`${collectionRecords.searchVector} @@ plainto_tsquery('simple', ${q})`,
    ];
    if (normalized.length > 0) {
      arms.unshift(
        sql`${collectionRecords.normalizedLabel} ILIKE ${`%${normalized}%`}`,
      );
    }
  ...
  if (filters.length > 0) {
    for (const f of filters) {
      const cond = buildFilterCondition(f, collectionId, fieldByKey.get(f.key));
      if (cond) conditions.push(cond);
    }
  }
  if (from) conditions.push(lt(collectionRecords.id, from));
  ...
  > => {
    ...
    if (!sortTarget) {
      ...
      return (
        db
          .select(getTableColumns(collectionRecords))
    ...
    return (
      db
        ...
        .from(collectionRecords)
        .innerJoin(
          sql`${sql.raw(qualifiedCollectionTable(collectionId))} ${sql.raw(EXT)}`,
          sql`${sql.raw(`${EXT}."id"`)} = ${collectionRecords.id}`,
        )
        ...
        .orderBy(...keys, desc(collectionRecords.id))
        .limit(limit)
        .offset(offset)
    );
  };
  ...
  const [dataById, computed] = await Promise.all([
    readRecordDataBatch({ collectionId, recordIds, fields: fieldDefs }),
