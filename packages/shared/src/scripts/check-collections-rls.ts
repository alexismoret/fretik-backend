...
import {
  dropCollectionTable,
  reconcileCollectionTable,
} from "../services/collection-schema/table";
 ...
 *
 * Two phases:
 *   1. STRUCTURAL (owner connection, read-only): every `data.coll_*` extension
 ...
 *      (`collection_records`, `collections`) keep their sharing-aware policies; the
...
const structural = async (): Promise<void> => {
  console.log(
    "\n[1/2] Structural — RLS armed on every data.coll_* + catalog\n",
  );
  ...
  const tables = await db.execute<{ tablename: string; rowsecurity: boolean }>(
    sql`SELECT tablename, rowsecurity FROM pg_tables
        WHERE schemaname = 'data' AND tablename LIKE 'coll\\_%' ESCAPE '\\'
        ORDER BY tablename`,
  );
  console.log(`Found ${tables.rows.length.toString()} data.coll_* table(s).`);
  ...
  for (const [tbl, policy, helper] of [
    ["collection_records", "sql_tool_team_isolation", "fretik_record_visible"],
    ["collections", "sql_tool_team_isolation", "fretik_type_granted"],
  ] as const) {
...
const buildFixture = async (orgId: string): Promise<Fixture> => {
  ...
  await reconcileCollectionTable({ collectionId: typeId });
  ...
  const tbl = qualifiedCollectionTable(typeId);
...
const teardown = async (fx: Fixture): Promise<void> => {
  ...
  await dropCollectionTable({ collectionId: fx.typeId }).catch(() => {});
...
const functional = async (readonlyUrl: string): Promise<void> => {
  ...
  const visibleAs = async (fx: Fixture, teamId: string): Promise<number> => {
    ...
    try {
      ...
      const r = await role.query<{ n: string }>(
        `SELECT count(*)::int AS n FROM ${qualifiedCollectionTable(fx.typeId)}`,
      );
  ...
  // Count the type's own CATALOG row (collections) AS the role — the gap the
  // grant-aware collections policy closes (data rows were already grant-aware).
  const typeRowVisibleAs = async (
    ...
    try {
      ...
      const r = await role.query<{ n: string }>(
        "SELECT count(*)::int AS n FROM collections WHERE id = $1",
        [fx.typeId],
      );
      return Number(r.rows[0]?.n ?? 0);
    } finally {
  ...
  try {
    ...
    await db
      ...
      .where(eq(collectionGrants.collectionId, fx.typeId));
    check(
      (await visibleAs(fx, fx.teamB)) === 0,
...
run().catch((error) => {
  console.error("[check-collections-rls] crashed:", error);
  process.exit(1);
});
