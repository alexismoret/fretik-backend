...
> **Fretik sandbox.** You work in `/workspace`; this skill lives in `skills/xlsx/`. Prefix every script path below with it (`python skills/xlsx/scripts/…`) or `cd skills/xlsx/` first. Python imports go through the loader: `from skill_loader import load_skill; load_skill("xlsx")`. Write deliverables under `outputs/` and surface them with `presentFiles({ paths: ["outputs/model.xlsx"] })`.
