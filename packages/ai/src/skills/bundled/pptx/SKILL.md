...
> **Fretik sandbox.** You work in `/workspace`; this skill lives in `skills/pptx/`. Prefix every script path below with it (`python skills/pptx/scripts/…`) or `cd skills/pptx/` first. Python imports go through the loader: `from skill_loader import load_skill; load_skill("pptx")`. Write deliverables under `outputs/` and surface them with `presentFiles({ paths: ["outputs/deck.pptx"] })`.
