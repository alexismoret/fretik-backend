...
| `TEAM_DIGEST_ENABLED`                                                    | ai. `true` (default, unset = this) · `false`. Whether the standing team digest is SERVED. `false` is the **rollback for the digest work**: `<team_digest>` renders its placeholder and recall stops suppressing the rows the digest covers — both, because suppressing without serving would leave those memories out of every block. The jobs keep rewriting the row, so turning it back on serves a CURRENT digest, and `team_memory_digests.previous_content` stays readable while it is off. Read at module load — next restart, not next turn. |
