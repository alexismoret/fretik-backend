...
3. `run_test` — a workflow CANNOT be activated until one test run has succeeded. Hand test files over via `files` (chat attachments never reach the run by themselves). The run then executes in the background: carry on with anything else this turn needs, then end it — never poll or sleep. This conversation is resumed on its own once every run it launched has finished. `get_run` returns the per-task detail AND the run's deliverables at `runs/<runId>/<file>`: OPEN them, and when the user gave an example of the output, **diff the two in `python`** — print, per column or field, the first place they differ. Reading them side by side does not work: a missing decimal, a prefix left on a label, a column empty in one and filled in the other all survive a careful read, twice over in prod. Classify each difference before acting: a structural or logic defect → fix with `update` and re-test; a value the input genuinely lacks (any correct run leaves that cell empty) → a finding to report, never a reason to re-run; a spec detail that could have been stated upfront (number format, label, sort order) → fold into the playbook with `update`, no new test — the deliverable already proved the logic. A test run costs the user minutes and money — five per conversation is the hard ceiling, the 3rd after a succeeded run requires an explicit `confirm: true`, and two rounds without convergence means showing the user the difference instead of firing a third.
...
- **The task that extracts is the task that opens the document.** A task whose only job is to sort, count or classify the inputs is not a task: fold it into the extraction task, which has to open the files anyway. Standing alone it reads every file whole, and that text then rides in the run's context for every later step — a 2026-07 run went from 38k to 101k input tokens per step that way, and the extraction task parsed the transcript instead of the file. If an inventory step is genuinely needed, give it the minimum: `bash: ls attachments`, the `<attached_file>` snapshot, at most one targeted read. An input that is not this workflow's at all never reaches a task — keeping it out is the `triggerCriterion`'s job. When a run can receive several kinds of document the workflow DOES handle, say that the task identifies each file and THEN extracts it with the fields that kind needs — otherwise the executor either tries every field set on every file, or builds one schema wide enough to cover them all and fills most of it with nulls. Identifying a file means a bounded look — its first page / ~50 lines — never "read the documents completely": `extract` reads the document itself and returns the structured data, so a full `read` on top sends every document through the run's context twice (a 2026-07-29 run paid 4 full reads AND 6 extracts over the same 3 files).
