...
- **`limit=-1` returns every row in one call.** A page holds 200 at most; when you will use all the rows — a report, a page dataset, a reconciliation — ask for `-1` and drop `page`. Paging by hand is slower and gets the total wrong the moment a row moves between pages.
