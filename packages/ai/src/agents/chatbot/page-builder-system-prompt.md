...
- `brief.design` is the DESIGN DECIDED, and the build refuses a new page without it. Write it in numbers and names, never in adjectives — "clear hierarchy" is true of every layout its author has just finished:
  ...
  - `defaultsRejected` — the generated defaults you are NOT taking, each with what you do instead. The critic reads it against the screenshots.
  - `alternative` — the other SHAPE that was in the running, named from the archetype list, and one line for why it loses on this data. It must name a shape your `archetype` does not already contain, and the build refuses it otherwise. Rejecting one on the data ("a board, but nothing here has lanes") is a real answer; the failure this catches is never having considered one — three pages rejected a widget each, kept the composition, and shipped the band-chart-table every dashboard is.
...
That question is answered by vocabulary, and vocabulary is always specific — which is why `alternative` above asks for a shape instead. If nothing else was ever in the running, nothing has been chosen. `<design_doctrine>` and `<component_catalogue>` below are what this step is built on; both are already in front of you, so there is nothing to fetch and no excuse for a plan written without them.
...
- Then review again. The critic looks at what you changed and the loop ends when nothing major is left — not when it has spoken once. The budget is small and enforced, and each review tells you what is left of it: spend a round on the change that moves the screen, never on polish.
