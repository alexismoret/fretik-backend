 ...
 * single-row `updateCollectionRecord` gets for free — but that is a Drizzle
...
export const buildRegistryUpdateBatch = (input: {
  ...
  return sql`UPDATE collection_records AS r
      SET label = v.label,
 ...
 */
export const bulkUpdateCollectionRecords = async (input: {
  teamId: string;
  ...
  for (const id of requestedIds) {
    ...
    if (!rec) continue;
    const arr = byType.get(rec.collectionId) ?? [];
    arr.push(id);
    byType.set(rec.collectionId, arr);
  }
