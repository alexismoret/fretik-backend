...
import {
  ...
  listSharedTypeIds,
  listTypeGrants,
} from "@fretik/shared/services/collection-sharing/list";
 ...
 * the single create/update path: `sharing` on `POST`/`PATCH /collections` and
 * `/objects` reconciles `collection_grants` / `record_shares` in the same
 * transaction, so there is no separate grant/share endpoint.
 */
const collectionSharingRoutes = new OpenAPIHono<HonoLoggedAppType>();
collectionSharingRoutes.use("*", authMiddleware);
...
collectionSharingRoutes.openapi(listTypeGrantsRoute, async (c) => {
...
collectionSharingRoutes.openapi(sharedRecordsRoute, async (c) => {
  const team = c.get("team");
  if (!team) return c.json(teamRequired(), 403);
...
export { collectionSharingRoutes };
