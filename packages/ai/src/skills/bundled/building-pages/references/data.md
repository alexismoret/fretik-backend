...
- **`external` reads the connected app directly, through the viewer's own connection — the default for a page over an outside system.** Nothing to maintain, fresh on every open. Ask the action for everything it can return in one call — its "every row" value, where the app's SKILL names one — and filter in the page rather than paging by hand: one answer carries up to 2 MB of rows and flags itself `truncated` if it had to drop any. A collection synced by a workflow is the LAST resort — a read too big for one answer, grouping the app cannot do server-side, or a join with workspace data — because it adds a collection and a workflow to maintain and every sync spends a run; say so in the summary when you reach for it. `pageProbe` shows the real answer shape first. A provider that answers one call at a time is slow by nature: give each dataset its own `datasetIds` query and render each as it lands.
