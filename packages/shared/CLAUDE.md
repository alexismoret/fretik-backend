...
- The old `entities` model (`entity_type`/`entity_role` enums) is **gone** — mentioned parties are now `collection_records` linked to the document mirror. The only transport heritage left: the `transport` document-field template (above) and the default mention-target type `company` in `services/documents/sync-document-graph.ts` (overridable per org via `organization_settings.document_mention_target_collection_key`). Keep new schema/service/lib code industry-agnostic.
...
- **Tests: read `backend/docs/OPERATIONS.md` §7 before writing one.** This package owns most of the SQL, so most of its tests are integration tests against a real Postgres — the sorting rule is _if the assertion still holds when you delete the `where` clause, the test is not testing the query_. `tests/lib/db-fixtures.ts` gives you `createWorkspaceFixture()`: an organization, a team, two users, row builders, and `createTeam()` for the second team a cross-tenant test needs (a whole second workspace differs in too many columns to prove anything). Three unit suites still fake `db` on purpose — one poison, two silencers — and `docs/TEST-TRIAGE.md` says why for each.
