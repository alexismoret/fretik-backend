...
| `STANDING_MODE`                                                          | ai. `episodes` (default, unset = this) · `none`. Whether `<standing_memory>` is served — the one memory block present on every turn and matched against nothing, rendered deterministically from recent episodes (team rows + the reader's OWN private ones, 30 days, ≤600 tokens). **`none` is the rollback** and is now a pure off switch: the block renders its placeholder and nothing else changes, because it no longer suppresses anything from `<active_memory>`. Read at module load — next restart, not next turn. Per-request, `/invoke` accepts `X-Standing-Mode` (400 on an unknown value). Replaced `TEAM_DIGEST_ENABLED` on 2026-09-11; the third arm, `digest`, was deleted the same day after the A/B (see `evals/RUNBOOK.md`). |
