...
export const createManageWorkflowTool = () =>
  tool({
    description: [
      ...
      "- A task that reads a lot to produce a little — research across sources, a set of documents to compare, one analysis per client or supplier — hints `dispatchAgent`: the executor hands the reading to sub-agents (in parallel when the task names independent items) and keeps only their reports, so a long run stays fast and its context small. Sub-agents only read and compute; the task's writes stay with the executor.",
