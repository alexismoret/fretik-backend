...
- `imap_smtp.send_email.op(to, subject, body_html, cc=None, bcc=None, attachments=None)` — Send a new email immediately (with optional attachments)
- `imap_smtp.reply_email.op(message_id, body_html, attachments=None)` — Reply to the sender of a message (preserves In-Reply-To threading)
- `imap_smtp.forward_email.op(message_id, to, comment=None, attachments=None)` — Forward a message to new recipients (with optional comment)
- `imap_smtp.mark_read.op(message_id)` — Mark a message as read (sets the \Seen flag)
- `imap_smtp.mark_unread.op(message_id)` — Mark a message as unread (clears the \Seen flag)
- `imap_smtp.delete_message.op(message_id)` — Move a message to Trash (or expunge if no Trash folder exists)
- `imap_smtp.move_message.op(message_id, destination_folder_id)` — Move a message to another folder
- `imap_smtp.delete_messages.op(message_ids)` — Delete multiple messages in a single IMAP batch (move to Trash, or expunge if no Trash exists)
- `imap_smtp.move_messages.op(message_ids, destination_folder_id)` — Move multiple messages to another folder in a single IMAP batch
- `imap_smtp.mark_messages_read.op(message_ids)` — Mark multiple messages as read (single IMAP STORE per source folder)
- `imap_smtp.mark_messages_unread.op(message_ids)` — Mark multiple messages as unread (single IMAP STORE per source folder)
- `imap_smtp.create_folder.op(display_name, parent_folder_id=None)` — Create a new mail folder
