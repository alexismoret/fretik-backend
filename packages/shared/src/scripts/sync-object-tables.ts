...
import { syncAllObjectTablesForTeam } from "../services/object-schema/catalog-sync";

/**
 * Idempotent maintenance script: reconcile every team's per-type extension
 * tables (`data.obj_<typeId>`) against their current field definitions —
 ...
 *
 * Run: `bun --env-file=../../.env run src/scripts/sync-object-tables.ts`
 */
const run = async (): Promise<void> => {
  ...
  for (const t of teams) {
    const count = await syncAllObjectTablesForTeam({
      organizationId: t.organizationId,
      teamId: t.id,
    });
    console.log(
      `[sync-tables] team ${t.id}: ${count.toString()} object table(s)`,
    );
  }
