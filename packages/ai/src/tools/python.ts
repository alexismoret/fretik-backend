...
export const createPythonTool = () =>
  tool({
    description: [
      ...
      "",
      "Reach for `python` whenever the work involves computing on structured or tabular data — parse the source programmatically rather than transcribing rows from a previous tool result. If you are about to embed values you just read into Python list/tuple literals, stop: a tabular source (CSV / XLSX / JSON) loads directly (`pandas`, `openpyxl`, `json`) into a variable; values locked in a document (PDF / DOCX / scan) come from `extract` as JSON you process here. Transcribed literals drift from the source as soon as anything is reformatted, and silently mis-match downstream. The same rule covers prose, not just data: a document-scale translation or rewrite is `transform`'s job — never author it as Python string literals.",
      "",
      "When the deliverable is a row set (CSV / JSON / DataFrame), completeness is verified, not assumed: get an independent count of the expected records from the source, reconcile it against your output row count, spot-check a few literal values, and flag any gap in the file — never ship a silently partial set.",
      "",
      ...
      "  - `skills/` — bundled skill bundles. Use `from skill_loader import load_skill; load_skill('<name>')` then import the script you need.",
      ...
      "- String matching: rapidfuzz — when joining records across files with formatting differences (accents, punctuation, abbreviations, suffix variation), use `rapidfuzz.process.extract` with `fuzz.ratio`; trial-join ~5 rows to pick a score threshold BEFORE the full join, then print matched/unmatched counts. Join on a normalized hidden key, never on the displayed value.",
      "",
