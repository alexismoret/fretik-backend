...
| `SEMANTIC_SCAN_MODE`                                                     | `hnsw` (default, unset = this) · `exact`. Which plan the semantic retrieval arm asks for. `exact` sets `enable_indexscan = off` for that statement, restoring the plan the arm ran before `hnsw.ef_search` reached 400 — the **rollback for the HNSW work**, no deploy, no migration to revert. Read at module load, so it takes effect on the next restart. See §9.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `STANDING_MODE`                                                          | ai. `digest` (default, unset = this) · `episodes` · `none`. Which implementation serves `<standing_memory>`, the one memory block that is present on every turn and matched against nothing. `episodes` renders recent episodes deterministically (team rows + the reader's own private ones, 30 days, ≤600 tokens); `digest` serves the generated `team_memory_digests` row. **`none` is the rollback**: the block renders its placeholder AND recall stops suppressing the rows the digest covers — both, because suppressing without serving would leave those memories out of every block. The jobs keep rewriting the digest row either way, so switching back to `digest` serves a CURRENT one, and `team_memory_digests.previous_content` stays readable throughout. Read at module load — next restart, not next turn. Per-request, `/invoke` accepts `X-Standing-Mode` (400 on an unknown value), which is how the three arms are compared against one live service instead of one restart apart. Replaced `TEAM_DIGEST_ENABLED` on 2026-09-11. |
| `MEMORY_DIGEST_DEBOUNCE_MS`                                              | jobs. Default `300000` (5 min). How long a team's digest rewrite waits after a memory write before it runs. The delay IS the debounce: every write re-enqueues the same `digest-<teamId>` job id, so a burst of writes costs one rewrite. Lower it and a team editing memories pays a model call per edit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
