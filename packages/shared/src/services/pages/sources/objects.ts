...
export const objectsSource: PageDataSource = {
  kind: "objects",
  resolve: async (
    dataset,
    { teamId, state, query },
    ...
    if (!dataset.objectTypeId) {
      return { status: "error", message: "dataset has no objectTypeId" };
    }

    const type = await db.query.objectTypes.findFirst({
      columns: { id: true },
      where: { id: dataset.objectTypeId },
    });
    ...
    const { count, data } = await listObjectRecords({
      teamId,
      objectTypeId: dataset.objectTypeId,
      filters,
      limit,
