...
import {
  analyzeObjectTable,
  estimatedRowCount,
} from "../object-schema/indexes";

/**
 * Which object types get a SEMANTIC card per record — the size policy behind
 ...
 *
 * What replaces semantic search above the ceiling: `object_records.search_vector`
...
const policyCacheKey = (objectTypeId: string): string =>
  `card-index-policy:${objectTypeId}`;
...
export const isCardIndexedType = async (
  ...
  selectOrCache(
    () => resolveCardIndexPolicy(objectTypeId),
    policyCacheKey(objectTypeId),
    POLICY_CACHE_TTL_S,
  );
...
const resolveCardIndexPolicy = async (
  ...
  let rows = await estimatedRowCount(objectTypeId);
  if (needsFreshEstimate({ preference, rows })) {
    await analyzeObjectTable(objectTypeId);
    rows = await estimatedRowCount(objectTypeId);
  }
...
export const forgetCardIndexPolicy = async (
  ...
  await redis.del(policyCacheKey(objectTypeId));
};
 ...
 *
 * Served by the GIN index on `metadata`: `object_type_id` appears only in
  ...
  const result = await (input.tx ?? db).execute(
    sql`DELETE FROM ${aiVectors}
        WHERE ${aiVectors.sourceType} = 'records'
          AND ${aiVectors.metadata} @> ${JSON.stringify({ object_type_id: input.objectTypeId })}::jsonb`,
  );
 ...
 * `object_records` row is unambiguously dead weight. It is not harmless: an
  ...
  const result = await db.execute(
    sql`DELETE FROM ${aiVectors} v
        WHERE v.source_type = 'records'
          AND NOT EXISTS (
            SELECT 1 FROM object_records r WHERE r.id = v.source_id
          )`,
  );
  return result.rowCount ?? 0;
};
...
export const reconcileCardIndexPolicy = async (input: {
  ...
  if (await isCardIndexedType(input.objectTypeId)) return 0;
  return purgeCardVectorsForType({ objectTypeId: input.objectTypeId });
};
