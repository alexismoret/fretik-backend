...
- **`external` reads the connected app directly, through the viewer's own connection — the default for a page over an outside system.** Nothing to maintain, fresh on every open. Ask the action for everything it can return in one call — its "every row" value, where the app's SKILL names one — and filter in the page rather than paging by hand: one answer carries up to 2 MB of rows and flags itself `truncated` if it had to drop any. A collection the app FILLS (`manageSync`) is the other half of the choice, not a last resort: it is what a read too big for one answer, grouping the app cannot do server-side, or a join with workspace data needs, and it is maintained by the platform rather than by a workflow. Prefer `external` for one thing, this moment, when the page is the only reader and the answer fits one call; prefer a synced collection when the team filters, joins or charts it, when several surfaces or a public link read it, or when the volume needs SQL. Say which you chose in the summary. `pageProbe` shows the real answer shape first. A provider that answers one call at a time is slow by nature: give each dataset its own `datasetIds` query and render each as it lands.
