/**
 * The `read` tool, over real `ai_chat_files` rows.
 *
 * An attachment is resolved by its STORED MIME — the row decides whether the
 * bytes come back as text, as extracted markdown, or as a steer to `python` or
 * `vision` — and the lookup that finds the row is scoped to the conversation.
 *
 * Until 2026-09-02 that lookup was a `db` Proxy keyed by
 * `${conversationId}::${filename}`: a JavaScript re-implementation of the
 * predicate, which is why every test gave itself a private conversation id
 * (`conv-a`, `conv-slice`, `conv-eof`, …). Those ids were keeping the FAKE's
 * map from colliding, not testing anything — the rows are now deleted between
 * tests, so one conversation does for all of them, and the second one below
 * exists to state the boundary the fake enforced for free.
 *
 * Doubled, all at process boundaries: `getOrCreateExtraction` (Mistral OCR),
 * `loadAccessibleContext` (its own service, with its own suite in
 * `@fretik/shared`), and the context-file S3 readers.
 */
import db from "@fretik/shared/db";
import { aiChatFiles } from "@fretik/shared/db/schema";
import {
  afterAll,
  beforeAll,
  beforeEach,
  describe,
  expect,
  test,
} from "bun:test";
import { inArray } from "drizzle-orm";
import { getProfileForRole } from "../../../src/lib/model-registry/resolve";
import { mockModule, mockModuleStrict } from "../../lib/mock-module";
import { installSandboxMocks, sandboxFs } from "../../lib/sandbox-fixture";
import type { MemoryTestFixture } from "../lib/db-fixtures";
import { createMemoryTestFixture } from "../lib/db-fixtures";

installSandboxMocks();

/** Extraction markdown keyed by fileHash (null markdown = image-skip). */
const extractions = new Map<string, string | null>();
/** Stored embedded-image ids keyed by fileHash ([] = legacy/no images). */
const extractionImageIds = new Map<string, string[]>();

await mockModuleStrict("@fretik/shared/services/file-extraction/extract", {
  getOrCreateExtraction: async (input: { fileHash: string }) => {
    const markdown = extractions.get(input.fileHash) ?? null;
    return {
      route: "mistral-ocr",
      markdown,
      pages: [],
      pageCount: null,
      charCount: markdown?.length ?? null,
      sidecarS3Key: null,
      imageIds: extractionImageIds.get(input.fileHash) ?? [],
    };
  },
});

// Context files are served Bun-side (no sandbox): the read tool loads the
// accessible set, then reads extracted markdown from the `content` column or
// the S3 sidecar — never a sandbox `.md`. Which files are ACCESSIBLE is
// `load-context`'s decision and has its own tests; here it is an input.
interface ContextFileRow {
  id: string;
  profileId: string;
  filename: string;
  mimeType: string;
  status: string;
  content: string | null;
}
const contextFiles: ContextFileRow[] = [];
const contextOriginals = new Map<string, Uint8Array>();
const contextSidecars = new Map<string, Uint8Array>();

await mockModule("../../../src/services/chatbot-context/load-context", {
  loadAccessibleContext: async () => ({
    userProfile: null,
    teamProfile: null,
    files: contextFiles.map((f) => ({ ...f, scope: "team" as const })),
  }),
});

// Only the two readers are backed by the in-memory maps; the writers and key
// builders keep their real implementations.
await mockModuleStrict("@fretik/shared/lib/ai-context-storage", {
  readContextOriginal: async (_profileId: string, fileId: string) =>
    contextOriginals.get(fileId) ?? null,
  readContextSidecar: async (_profileId: string, fileId: string) =>
    contextSidecars.get(fileId) ?? null,
  uploadContextSidecar: async () => undefined,
  deleteContextOriginal: async () => undefined,
  deleteContextSidecar: async () => undefined,
});

const { createReadTool } = await import("../../../src/tools/read");
const { DynamicToolManager } =
  await import("../../../src/agents/shared/dynamic-tools");
const { wrapRuntimeContext } =
  await import("../../../src/agents/shared/runtime-context");

type ExecuteOptions = {
  toolCallId: string;
  messages: never[];
  /**
   * `Record<string, unknown>` — the SDK's `Context` — and not `unknown`: the
   * options bag is handed straight to `tool.execute`, which will not accept a
   * context it cannot read.
   */
  context: Record<string, unknown>;
};

let fx: MemoryTestFixture;
/** The conversation every test reads in. */
let conv: string;
/** A second conversation in the same workspace — the neighbour. */
let otherConv: string;

beforeAll(async () => {
  fx = await createMemoryTestFixture();
  conv = await fx.createConversation();
  otherConv = await fx.createConversation();
});

afterAll(async () => {
  await fx.cleanup();
});

const isRecord = (v: unknown): v is Record<string, unknown> =>
  typeof v === "object" && v !== null;

const readString = (record: Record<string, unknown>, key: string): string => {
  const value = record[key];
  return typeof value === "string" ? value : "";
};

const buildExecuteOptions = (conversationId: string): ExecuteOptions => {
  const ctx = {
    organizationId: fx.organizationId,
    teamId: fx.teamId,
    conversationId,
    modelProfile: getProfileForRole("chat"),
    dynamicToolManager: new DynamicToolManager(),
  };
  return {
    toolCallId: `tc-${Date.now().toString()}-${Math.random().toString(36).slice(2, 8)}`,
    messages: [],
    context: wrapRuntimeContext(ctx),
  };
};

const execRead = async (
  file_path: string,
  opts: { offset?: number; limit?: number; conversationId?: string } = {},
): Promise<unknown> => {
  const { conversationId = conv, ...input } = opts;
  const tool = createReadTool();
  if (typeof tool.execute !== "function") {
    throw new Error("read tool missing execute");
  }
  return await tool.execute(
    { file_path, ...input },
    buildExecuteOptions(conversationId),
  );
};

const expectRecord = (result: unknown): Record<string, unknown> => {
  if (!isRecord(result)) {
    throw new Error(`read returned non-object: ${JSON.stringify(result)}`);
  }
  return result;
};

/**
 * Seed a chat attachment: insert its `ai_chat_files` row and back it with
 * content — S3 bytes for text MIMEs, an extraction markdown entry for
 * documents/images. `extractionMarkdown: null` simulates an image with no
 * usable text (image-skip).
 */
const seedAttachment = async (args: {
  filename: string;
  mimeType: string;
  conversationId?: string;
  fileHash?: string;
  textContent?: string;
  extractionMarkdown?: string | null;
  /** Stored embedded-image ids (omit = legacy/no images). */
  imageIds?: string[];
}): Promise<void> => {
  const conversationId = args.conversationId ?? conv;
  const fileHash = args.fileHash ?? `hash-${args.filename}`;
  await db.insert(aiChatFiles).values({
    conversationId,
    filename: args.filename,
    mimeType: args.mimeType,
    size: args.textContent?.length ?? 1024,
    fileHash,
  });
  if (args.textContent !== undefined) {
    sandboxFs.seedS3(
      conversationId,
      `attachments/${args.filename}`,
      args.textContent,
    );
  }
  if (args.extractionMarkdown !== undefined) {
    extractions.set(fileHash, args.extractionMarkdown);
  }
  if (args.imageIds !== undefined) {
    extractionImageIds.set(fileHash, args.imageIds);
  }
};

beforeEach(async () => {
  sandboxFs.reset();
  extractions.clear();
  extractionImageIds.clear();
  contextFiles.length = 0;
  contextOriginals.clear();
  contextSidecars.clear();
  await db
    .delete(aiChatFiles)
    .where(inArray(aiChatFiles.conversationId, [conv, otherConv]));
});

describe("read tool — path sandbox", () => {
  test("rejects an absolute path outside /workspace/", async () => {
    const out = expectRecord(await execRead("/etc/passwd"));
    expect(out["code"]).toBe("PATH_OUT_OF_SANDBOX");
    expect(readString(out, "error")).toContain("sandbox");
  });

  test("rejects traversal via ../", async () => {
    const out = expectRecord(await execRead("../other-conv/secret.md"));
    expect(out["code"]).toBe("PATH_OUT_OF_SANDBOX");
  });

  test("accepts absolute /workspace/ attachment path", async () => {
    await seedAttachment({
      filename: "note.md",
      mimeType: "text/markdown",
      textContent: "hello",
    });
    const out = expectRecord(await execRead("/workspace/attachments/note.md"));
    expect(out["source"]).toBe("original");
    expect(readString(out, "content")).toContain("hello");
  });
});

describe("read tool — attachments (transparent)", () => {
  test("bare basename is auto-resolved under attachments/", async () => {
    await seedAttachment({
      filename: "shipping-notes.md",
      mimeType: "text/markdown",
      textContent: "first\nsecond\nthird",
    });
    const out = expectRecord(await execRead("shipping-notes.md"));
    expect(out["source"]).toBe("original");
    expect(out["numLines"]).toBe(3);
    expect(out["totalLines"]).toBe(3);
    const content = readString(out, "content");
    expect(content).toContain("     1\tfirst");
    expect(content).toContain("     3\tthird");
  });

  test("offset + limit slice returns real file line numbers", async () => {
    await seedAttachment({
      filename: "long.md",
      mimeType: "text/markdown",
      textContent: "alpha\nbravo\ncharlie\ndelta\necho",
    });
    const out = expectRecord(
      await execRead("attachments/long.md", { offset: 3, limit: 2 }),
    );
    expect(out["startLine"]).toBe(3);
    expect(out["numLines"]).toBe(2);
    expect(out["totalLines"]).toBe(5);
    const content = readString(out, "content");
    expect(content).toContain("     3\tcharlie");
    expect(content).toContain("     4\tdelta");
    expect(content).not.toContain("echo");
  });

  test("offset past EOF returns numLines=0 without erroring", async () => {
    await seedAttachment({
      filename: "short.md",
      mimeType: "text/markdown",
      textContent: "only",
    });
    const out = expectRecord(
      await execRead("attachments/short.md", { offset: 99 }),
    );
    expect(out["numLines"]).toBe(0);
    expect(out["totalLines"]).toBe(1);
    expect(out["content"]).toBe("");
  });

  test("PDF is read transparently as the extracted text", async () => {
    await seedAttachment({
      filename: "invoice.pdf",
      mimeType: "application/pdf",
      extractionMarkdown: "Invoice #42 — Total: 100 EUR",
    });
    const out = expectRecord(await execRead("attachments/invoice.pdf"));
    expect(out["source"]).toBe("original");
    expect(readString(out, "content")).toContain("Invoice #42");
  });

  test("DOCX is read transparently as the extracted text", async () => {
    await seedAttachment({
      filename: "contract.docx",
      mimeType:
        "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      extractionMarkdown: "## Agreement\n\nParty A and Party B",
    });
    const out = expectRecord(await execRead("attachments/contract.docx"));
    expect(out["source"]).toBe("original");
    expect(readString(out, "content")).toContain("Agreement");
  });

  test("image with usable text returns the extracted text", async () => {
    await seedAttachment({
      filename: "receipt.jpg",
      mimeType: "image/jpeg",
      extractionMarkdown: "Receipt\nTotal: 12.50 EUR",
    });
    const out = expectRecord(await execRead("attachments/receipt.jpg"));
    expect(out["source"]).toBe("original");
    expect(readString(out, "content")).toContain("Receipt");
  });

  test("image with no extractable text returns a vision hint", async () => {
    await seedAttachment({
      filename: "cat.jpg",
      mimeType: "image/jpeg",
      extractionMarkdown: null,
    });
    const out = expectRecord(await execRead("attachments/cat.jpg"));
    expect(out["code"]).toBe("NO_TEXT_CONTENT");
    expect(out["hint"]).toBe("vision");
  });

  test("XLSX routes to python (no text dump)", async () => {
    await seedAttachment({
      filename: "data.xlsx",
      mimeType:
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    });
    const out = expectRecord(await execRead("attachments/data.xlsx"));
    expect(out["code"]).toBe("BINARY_NOT_READABLE");
    expect(out["hint"]).toBe("python");
  });

  test("missing attachment row returns FILE_NOT_FOUND", async () => {
    const out = expectRecord(await execRead("attachments/ghost.md"));
    expect(out["code"]).toBe("FILE_NOT_FOUND");
  });

  test("an attachment of ANOTHER conversation is not readable here", async () => {
    // The claim the per-test conversation ids used to hide: the row exists,
    // its bytes exist, and the only thing between this turn and someone else's
    // upload is `conversation_id` in the WHERE.
    await seedAttachment({
      conversationId: otherConv,
      filename: "neighbour.md",
      mimeType: "text/markdown",
      textContent: "not yours",
    });

    const out = expectRecord(await execRead("attachments/neighbour.md"));

    expect(out["code"]).toBe("FILE_NOT_FOUND");
    expect(readString(out, "content")).not.toContain("not yours");
  });

  test("the same filename in two conversations reads each its own", async () => {
    // And the other half: scoping must not turn into refusing everything.
    await seedAttachment({
      filename: "notes.md",
      mimeType: "text/markdown",
      textContent: "mine",
    });
    await seedAttachment({
      conversationId: otherConv,
      filename: "notes.md",
      mimeType: "text/markdown",
      textContent: "theirs",
    });

    const here = expectRecord(await execRead("attachments/notes.md"));
    const there = expectRecord(
      await execRead("attachments/notes.md", { conversationId: otherConv }),
    );

    expect(readString(here, "content")).toContain("mine");
    expect(readString(here, "content")).not.toContain("theirs");
    expect(readString(there, "content")).toContain("theirs");
  });
});

describe("read tool — extracted figures", () => {
  test("figure refs are rewritten to virtual paths when images are stored", async () => {
    await seedAttachment({
      filename: "report.pdf",
      mimeType: "application/pdf",
      extractionMarkdown: "Intro\n\n![chart](img-0.jpeg)\n\n![logo](img-1.png)",
      imageIds: ["img-0.jpeg"],
    });
    const out = expectRecord(await execRead("attachments/report.pdf"));
    const content = readString(out, "content");
    // Manifest id rewritten; non-stored id untouched.
    expect(content).toContain("attachments/report.pdf/img-0.jpeg");
    expect(content).toContain("![logo](img-1.png)");
    expect(content).not.toContain("attachments/report.pdf/img-1.png");
  });

  test("legacy extraction (no stored images) keeps refs untouched", async () => {
    await seedAttachment({
      filename: "old.pdf",
      mimeType: "application/pdf",
      extractionMarkdown: "![chart](img-0.jpeg)",
    });
    const out = expectRecord(await execRead("attachments/old.pdf"));
    expect(readString(out, "content")).toContain("![chart](img-0.jpeg)");
    expect(readString(out, "content")).not.toContain(
      "attachments/old.pdf/img-0.jpeg",
    );
  });

  test("read on a figure path steers to vision", async () => {
    await seedAttachment({
      filename: "report.pdf",
      mimeType: "application/pdf",
      extractionMarkdown: "![chart](img-0.jpeg)",
      imageIds: ["img-0.jpeg"],
    });
    const out = expectRecord(
      await execRead("attachments/report.pdf/img-0.jpeg"),
    );
    expect(out["code"]).toBe("NO_TEXT_CONTENT");
    expect(out["hint"]).toBe("vision");
    expect(readString(out, "error")).toContain("vision(");
  });
});

describe("read tool — non-attachment workspace paths", () => {
  test("explicit subdir paths read directly (outputs/)", async () => {
    sandboxFs.write(conv, "outputs/report.md", "alpha\nbravo");
    const out = expectRecord(await execRead("outputs/report.md"));
    expect(out["source"]).toBe("original");
    expect(out["totalLines"]).toBe(2);
  });

  test("persisted-output txt under outputs/persisted/ reports source=persisted-output", async () => {
    sandboxFs.write(conv, "outputs/persisted/tc-abc123.txt", "one\ntwo\nthree");
    const out = expectRecord(await execRead("outputs/persisted/tc-abc123.txt"));
    expect(out["source"]).toBe("persisted-output");
  });

  test("context/<file>.pdf serves the extracted markdown Bun-side (no sandbox)", async () => {
    contextFiles.push({
      id: "ctx-1",
      profileId: "profile-1",
      filename: "handbook.pdf",
      mimeType: "application/pdf",
      status: "ready",
      content: "Internal handbook contents",
    });
    const out = expectRecord(await execRead("context/handbook.pdf"));
    expect(out["source"]).toBe("original");
    expect(readString(out, "content")).toContain("Internal handbook");
  });

  test("context/<file>.xlsx routes to python instead of dumping text", async () => {
    contextFiles.push({
      id: "ctx-2",
      profileId: "profile-1",
      filename: "grid.xlsx",
      mimeType:
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      status: "ready",
      content: null,
    });
    const out = expectRecord(await execRead("context/grid.xlsx"));
    expect(out["code"]).toBe("BINARY_NOT_READABLE");
    expect(readString(out, "hint")).toBe("python");
  });
});

describe("read tool — persisted-output handoff", () => {
  test("oversized content is wrapped in a <persisted-output> envelope", async () => {
    // Many short lines: the 30K byte cap trims the join, then cat -n
    // line-numbering inflates it past the 120K persist threshold.
    const huge = Array.from({ length: 30_000 }, () => "x").join("\n");
    await seedAttachment({
      filename: "big.md",
      mimeType: "text/markdown",
      textContent: huge,
    });
    const out = await execRead("attachments/big.md", { limit: 30_000 });
    expect(typeof out).toBe("string");
    const envelope = out as string;
    expect(envelope).toContain("<persisted-output>");
    expect(envelope).toContain("Output too large");
    expect(envelope).toContain("outputs/persisted/");
    expect(envelope).not.toContain("/tmp/");
  });
});
