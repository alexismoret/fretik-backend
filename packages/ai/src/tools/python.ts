...
export const createPythonTool = () =>
  tool({
    description: [
      ...
      "",
      "Reach for `python` whenever the work involves computing on structured or tabular data — parse the source programmatically rather than transcribing rows from a previous tool result. If you are about to embed values you just read into Python list/tuple literals, stop: a tabular source (CSV / XLSX / JSON) loads directly (`pandas`, `openpyxl`, `json`) into a variable; values locked in a document (PDF / DOCX / scan) come from `extract` as JSON you process here. Transcribed literals drift from the source as soon as anything is reformatted, and silently mis-match downstream.",
      "",
