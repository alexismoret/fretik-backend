...
export const PAGE_ENVIRONMENT_GUIDE = [
  ...
  'A page may also have VIEWS of its own, each with an address someone can send: `pages/index.vue` answers at `/`, `pages/deal.vue` (or `pages/deal/index.vue`) at `/deal`, `pages/deal/[id].vue` at `/deal/:id` — Nuxt\'s convention, one level of nesting, a `[param]` only below a named directory. Declaring any of them makes `Page.vue` the SHELL — what every view shares (loading, nav, breadcrumb) — and it must render `<RouterView />` where the current view goes, or the page shows nothing. A view reads its own address with `useRoute().params.id` and asks for its data with it. Navigate with `<ULink to="/deal/7">`, `router.push`, or a plain `<a href="/deal/7">`; `router.replace({ query: { tab } })` for a filter that should not cost a back press. The app\'s url carries the view, so the link a reader copies reopens exactly this one.',
  "",
  "## imports",
  "Exactly these, nothing else (the compiler refuses others by name): `vue`, `vue-router` (`useRoute`, `useRouter` — `RouterView` and `RouterLink` are registered, so they need no import), `@nuxt/ui`, `chart.js` (or `chart.js/auto`, pre-registered), `#fretik/sdk`, `@vueuse/core` (curated: `useInfiniteScroll`, `useVirtualList`, `useElementSize`, `useDebounceFn`, `onClickOutside` and neighbours — storage, fetch and clipboard composables are absent), `@internationalized/date` (the value type `UCalendar`/`UInputDate`/`UInputTime` take — never a `Date`), and drag-and-drop: `@atlaskit/pragmatic-drag-and-drop/element/adapter`, `/combine`, `/reorder`, `@atlaskit/pragmatic-drag-and-drop-hitbox/closest-edge`. A relative path reaches the project's own files; anything else must be on that list.",
  ...
  "`fretik.context` — reactive `{ dark, locale, mode, variables, route }`. Colors/dark-mode are synced automatically; read `dark`/`locale`/`mode` only when the CODE must branch. `route.fullPath` is where the reader arrived, already applied by the router at mount — read it only to answer a question the router cannot, never to route by hand.",
