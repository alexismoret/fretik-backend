 ...
 *
 * One indexed query: `object_records ⋈ data.obj_<typeId> ⋈ locations`, filtered
 ...
 * `data.obj_<typeId>`, which is created by the DDL engine and is NOT a Drizzle
...
export const getMapPoints = async (input: {
  ...
  const table = sql.raw(qualifiedObjectTable(input.objectTypeId));
  ...
  const fromWhere = sql`
    FROM object_records
    JOIN ${table} o ON o.id = object_records.id
    JOIN public.locations loc ON loc.id = ${fkCol}
    WHERE object_records.object_type_id = ${input.objectTypeId}
      AND object_records.status = 'confirmed'
