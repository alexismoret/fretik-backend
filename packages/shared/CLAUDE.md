...
- **Agent-facing prose:** the few prompt strings owned by this package (`src/services/pre-extract/prompt.ts`, `src/services/compaction/prompt.ts`) follow `.agent/agent-facing-prose.md`. Same rule as `@fretik/ai`: compact, imperative, no hedging, no duplication.
