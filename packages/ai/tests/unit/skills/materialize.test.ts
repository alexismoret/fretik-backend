 ...
 *
 * Filesystem-only, and a unit test again. It lived under
 * `tests/integration/` for a reason that no longer exists: importing
 * `src/skills/materialize` reaches `@fretik/shared/db` transitively, and
 * that import used to APPLY MIGRATIONS at module load — so loading the
 * subject demanded a live Postgres. Importing the database is inert now.
 *
