 ...
 * `data.obj_<typeId>`. The rest of the codebase keeps speaking `data:
...
export const buildExtensionInsert = (input: {
  ...
  return sql`INSERT INTO ${sql.raw(qualifiedObjectTable(input.objectTypeId))} (${sql.raw(
    cols.join(", "),
 ...
 * `buildExtensionInsert`, used by `bulkCreateObjectRecords` so N records of one
...
export const buildExtensionUpdate = (input: {
  ...
  return sql`UPDATE ${sql.raw(qualifiedObjectTable(input.objectTypeId))} SET ${sql.join(
    sets,
    sql`, `,
 ...
 * by `bulkUpdateObjectRecords`. Every row sets `label` + every scalar column
...
export const buildExtensionUpdateBatch = (input: {
  ...
  return sql`UPDATE ${sql.raw(qualifiedObjectTable(input.objectTypeId))} AS e
...
export const readRecordData = async (input: {
  ...
  const res = await exec.execute(
    sql`SELECT ${sql.raw(expr)} AS data FROM ${sql.raw(
      qualifiedObjectTable(input.objectTypeId),
