...
/** Abandon a handshake that is not going to complete. */
const CONNECT_TIMEOUT_MS = 20_000;

/**
 * Open a session, run `fn`, close the session whatever happens.
 *
...
const openFtpSession = async (
  ...
  return {
    ...
    upload: async (path, bytes) => {
      await client.uploadFrom(Readable.from(Buffer.from(bytes)), path);
    },
    ...
    ensureDirectory: async (path) => {
      ...
      // directory keeps every subsequent relative command honest.
    ...
    chmod: async (path, mode) => {
      ...
      const response = await client
        .sendIgnoringError(`SITE CHMOD ${mode} ${path}`)
        .catch(() => null);
...
const toBytes = (value: string | Buffer | NodeJS.WritableStream): Uint8Array =>
