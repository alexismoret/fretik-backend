...
/**
 * Withdraw a pending invitation. Whoever may invite into its team may take
 * one back (`members.invite` there); one with no team is an invitation into
 * the organization alone, and the admins' (`members.manage`).
 *
...
export const cancelInvitation = async (input: {
  ...
  const row = await db.query.invitation.findFirst({
    columns: { id: true, email: true, teamId: true, status: true },
  ...
  await recordAccessEvent({
    ...
    metadata: { email: row.email, teamId: row.teamId },
  });
};
