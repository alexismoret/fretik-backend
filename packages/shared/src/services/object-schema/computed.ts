...
import { assertSafeKey, qualifiedObjectTable, SAFE_IDENT } from "./identifiers";
 ...
 * relation chips + rollup cells, and the AI `getObject` tool).
 *
...
const rollupProjection = (
  ...
  const targetJoin =
    targetFieldKey && targetTypeId
      ? `JOIN ${qualifiedObjectTable(targetTypeId)} te ON te.id = rt.id`
      : "";
  const targetCol =
...
/** linkType.normalizedKey → toObjectTypeId for the team (rollup target join). */
const loadLinkTargets = async (
  exec: Executor,
  teamId: string,
): Promise<LinkTargetMap> => {
  const rows = await exec
    .select({ key: linkTypes.normalizedKey, target: linkTypes.toObjectTypeId })
    .from(linkTypes)
