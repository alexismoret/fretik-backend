 ...
 */
export const indexName = (objectTypeId: string, column: string): string => {
  const tail = objectTableName(objectTypeId).slice("obj_".length);
...
export const estimatedRowCount = async (
  ...
  const rows = await db.execute<{ n: number }>(
    sql`SELECT GREATEST(c.reltuples, 0)::bigint::int AS n
        ...
        WHERE n.nspname = 'data' AND c.relname = ${objectTableName(objectTypeId)}`,
  );
 ...
 */
export const analyzeObjectTable = async (
  objectTypeId: string,
): Promise<void> => {
  await db.execute(sql.raw(`ANALYZE ${qualifiedObjectTable(objectTypeId)}`));
};
...
export const ensureColumnIndex = async (input: {
  ...
  const name = indexName(input.objectTypeId, input.column);
