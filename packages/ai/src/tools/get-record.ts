 ...
 * linked records (both directions) grouped by relation. Use after `listRecords`
 * or a `querySql` that returned a record id, when the user wants the full
 * record + what it is connected to.
 */
export const createGetRecordTool = () =>
  tool({
    description: [
      "Fetch one record by id (team-scoped): its fields plus the records it links to and from, grouped by relation.",
      "",
      "Use after `listRecords` or a `querySql` that surfaced a record id, when the user wants the full record and its connections (e.g. a company and the documents that mention it). Returns id, label, status, data, and outgoing/incoming links (relation key + the record on the other end).",
    ].join("\n"),
    inputSchema: z.object({
      id: z
        .string()
        .uuid()
        .describe("Record id from listRecords or a querySql result."),
    }),
