...
describe("entities reaching the digest", () => {
  ...
  test("both endpoints of an edge state the SAME triple, in its true direction", async () => {
    ...
    //
    // The subject is named on every line rather than implied by an arrow's
    // polarity, because the polarity version still lost one generation in
    // fourteen on exactly this line.
    ...
    const triple = "Vendor A — supplies → Project B";
    expect(subject?.links).toEqual([triple]);
    expect(object?.links).toEqual([triple]);
  });
});
