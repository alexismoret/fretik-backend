 ...
 * typed columns on the per-type extension table (`data.coll_<collectionId>`), so adding
...
export const recomputeSearchVectorsForType = async (input: {
  ...
  const res = await exec.execute(
    sql`UPDATE collection_records r
        SET search_vector = to_tsvector('simple', ${sql.raw(textExpr)})
        FROM ${sql.raw(table)} e
        WHERE e.id = r.id
          AND r.collection_id = ${input.collectionId}::uuid
