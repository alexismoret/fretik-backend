 ...
 * schemas can never drift from production. `dispatchAgent`, `manageAgents`
 * and `buildPage` import their schemas directly (they are registered beside
 * the core and domain sets, not in them; the schemas are hoisted precisely
 * for this consumer).
 *
...
import { manageAgentsInputSchema } from "../src/tools/manage-agents";
...
const buildSchemaMap = (): Map<string, ZodLike> => {
  ...
  map.set("manageAgents", manageAgentsInputSchema);
  // Same reason: registered outside the two sets, so the schema is hoisted
  // for this consumer.
