...
export const reconcileFieldIndexes = async (input: {
  ...
  let rows = await estimatedRowCount(input.collectionId);
  if (rows < INDEX_ROW_THRESHOLD) {
    ...
    await analyzeCollectionTable(input.collectionId);
    rows = await estimatedRowCount(input.collectionId);
  ...
  const existing = new Set(
    (await listAutoIndexUsage(input.collectionId)).map((index) => index.name),
  );
  let created = 0;
  for (const target of targets) {
    if (existing.has(indexName(input.collectionId, target.column))) continue;
...
export const pruneUnusedFieldIndexes = async (input: {
  ...
  const usage = await listAutoIndexUsage(input.collectionId);
  ...
  const byIndexName = new Map(
    indexTargetsForType(fields).map((target) => [
      indexName(input.collectionId, target.column),
      target.fieldId,
    ]),
  );
 ...
 * (`coll_<32 hex>`), so the uuid is rebuilt here rather than by loading every
 * collection and filtering in memory.
 */
export const collectionsWorthIndexing = async (
  limit = 200,
): Promise<string[]> => {
  const rows = await db.execute<{ id: string }>(
    sql`SELECT substring(h, 1, 8) || '-' || substring(h, 9, 4) || '-'
        ...
        FROM (
          SELECT substring(c.relname from 6) AS h, c.reltuples AS n
          FROM pg_class c
          JOIN pg_namespace ns ON ns.oid = c.relnamespace
          WHERE ns.nspname = 'data'
            AND c.relkind = 'r'
            AND c.relname ~ '^coll_[0-9a-f]{32}$'
            AND (
              ...
              OR EXISTS (
                ...
                WHERE i.relid = c.oid
                  AND i.indexrelname ~ ('^ix_' || substring(c.relname from 6) || '_[0-9a-f]{8}$')
              )
            )
        ) t
        ORDER BY n DESC
