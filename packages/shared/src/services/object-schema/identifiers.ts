...
/**
 * Physical table name for an object type: `obj_<objectTypeId-hex>`. Opaque and
 ...
 * `data` schema; the qualified form is `data.obj_<hex>`.
 */
export const objectTableName = (objectTypeId: string): string => {
  assertSafeUuid(objectTypeId, "object type id");
  return `obj_${hex(objectTypeId)}`;
};
